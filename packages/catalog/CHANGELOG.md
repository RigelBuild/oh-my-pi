...
- Fixed Claude Opus 5.5 replaying signed thinking after a conversation-prefix change on first-party, Vertex, and Cloudflare AI Gateway: those rows now carry prefix-bound thinking like Fable 5.1 and opt into dropping mismatched thinking blocks instead of failing with an invalid-signature 400. Other hosts are unchanged until their support for the controls beta is verified.
