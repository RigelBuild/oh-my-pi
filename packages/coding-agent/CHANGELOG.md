...
- The auth-broker resolves `auth.broker.metrics` from the agent directory instead of the launch directory, so a checked-in project `.omp/config.yml` can no longer enable the `/metrics` endpoint — project settings merge over global, so it could previously override an explicit global `false` ([#10290](https://github.com/can1357/oh-my-pi/pull/10290) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
