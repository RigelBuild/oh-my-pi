...
- Added `/mcp refresh` to re-fetch tools from connected MCP servers without reconnecting.

### Fixed

- Fixed MCP tools staying empty for the whole session when a server first listed no tools during its warmup window.
