...
- Fixed the in-session `refresh` so an MCP refresh keeps extension-declared servers and reloads `.mcp.json` from the session's current directory after a `/move` (instead of respawning the previous project's servers), a settings refresh that changes browser prelude enablement reconciles browser MCP servers against that same current directory instead of loading and spawning the previous project's, a rules refresh (including TTSR) reaches running descendant sessions without a subagent's agent-scoped roster replacing the process-global skills/rules snapshot — nor does a subagent's settings-only refresh that changes `ttsr.disabledRules`/`ttsr.builtinRules`, which applies to the child's own rules while leaving the snapshot contextless `rule://` consumers read to the top-level session — a startup that supplied a model but no thinking level leaves the level following `defaultThinkingLevel` so a later settings refresh can still move it, reloaded generation and prompt-affecting settings reach the live agent, and a settings refresh discloses that it may reconnect MCP servers ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
...
- Fixed `providers.webSearchOrder`, `providers.webSearchExclude`, and `providers.imageOrder` updating only the `Settings` view on `/refresh settings`; the search and image implementations read module-level state, so the centralized provider-globals helper is reapplied when one of these moves and searches no longer keep the old order or exclusions. That reapplication is now scoped to the refreshing session: web search and image generation resolve their provider order and exclusions from the calling session's own settings, so with several top-level sessions in one process, refreshing one no longer redirects another's searches or image requests to a provider it never selected. Callers without settings of their own, such as the one-shot search CLI, keep using the process-wide values ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
...
- Fixed `/refresh settings` overwriting an explicit thinking-level selection that happened to match the level already active; because the selection changed no effective effort it recorded no pin, so a later configured-default move replaced the user's choice. Selecting only a MODEL no longer pins thinking either: when the newly selected model carried its own default thinking level, applying it was recorded as an explicit thinking choice, so a later `defaultThinkingLevel` edit plus `/refresh settings` could no longer move the level even though the user had chosen only a model. A per-family service tier that `/refresh settings` updates now also survives switching sessions away and back, or restarting and resuming, instead of reverting to its pre-refresh value once another family had been set with `/fast` or the settings selector. A resumed session whose last model pick came from an older build's model cycling — which recorded no role on the transition — likewise no longer has that pick replaced by the configured default: a role-less transition written after the session's first message is read as the user's, while one preceding every message is the startup receipt and still follows settings. A session that went through a retry model fallback also keeps following settings: the fallback's automatic thinking adjustment is no longer recorded as a user selection, so editing `defaultThinkingLevel` and running `/refresh settings` still moves the level after a failover, while a thinking level the user really did pick before the failover survives it. Resuming a transcript from an older build no longer freezes its thinking level either: those startup and per-turn `auto` receipts carry no provenance at all, and were read as explicit pins, so a resumed session ignored every later `defaultThinkingLevel` edit. `/refresh` also sanitizes an invalid scope argument before echoing it, so a pasted or ACP-supplied value carrying ANSI escapes, control bytes, or an oversized payload can no longer corrupt the terminal ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- `/refresh settings` now reconciles every built-in tool whose existence is gated on a setting, not just the image and speech tool sets: disabling `bash.enabled`, `grep.enabled`, `glob.enabled`, `github.enabled`, `debug.enabled`, `lsp.enabled`, `web_search.enabled`, `security.enabled`, `ask.enabled`, `todo.enabled`, `astGrep.enabled`, `astEdit.enabled`, `checkpoint.enabled`, or `autolearn.enabled` on disk removes the tool from the live session instead of leaving it advertised and callable, and enabling one that was absent at startup constructs it. A tool the session was never granted — omitted from an explicit tool whitelist, or any tool under a restricted tool set — still stays absent. Live state is never migrated: if the group being swapped owns running work (a bash command or background bash job, an active debug session, an open checkpoint), that group alone is left untouched and the refresh reports an error naming the blocker so it can be stopped and the refresh retried, while every other group still reconciles in the same pass.
