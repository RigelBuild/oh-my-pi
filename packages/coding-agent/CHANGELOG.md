...
- Fixed two more OpenAI Responses vision requests still hitting the request-size (413) error with no image dropped: a replayed percent-encoded image data URI was charged its smaller decoded size while its larger `%`-escaped text travelled on the wire, and a replayed tool call with invalid arguments was counted as pairing a later image result even though it is stripped before the request is built, leaving that result's bytes uncharged. Both are now measured by what the request actually carries ([#10286](https://github.com/can1357/oh-my-pi/pull/10286) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed another OpenAI Responses vision request still hitting the request-size (413) error with no image dropped: when a replayed user or developer payload put an older small generated image before a newer oversized input image, the byte budget sized its single eviction against the large image but spent it on the small one, leaving the large image on the wire; the payload's images are now measured in the same order they are evicted ([#10286](https://github.com/can1357/oh-my-pi/pull/10286) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
- Fixed an OpenAI Responses session staying unable to send after a replayed computer-use screenshot was corrupt: the undecodable-image guard checked only `input_image` parts, so a malformed `computer_call_output` screenshot on a warm or compaction replay was sent unchanged and rejected on every request. The guard now drops the bad inline screenshot, keeps a `file_id` reference when one exists, and otherwise removes the output with its paired call ([#10286](https://github.com/can1357/oh-my-pi/pull/10286) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
