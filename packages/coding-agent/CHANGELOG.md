...
- Fixed a `/refresh` that pauses in an async host pre-refresh hook and is disposed before it resumes still overwriting the process-wide active skill/rule snapshot, so a torn-down session could clobber the roster another running session had already published; the refresh now rechecks disposal after the hook and abandons the reload ([#10288](https://github.com/can1357/oh-my-pi/pull/10288) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
