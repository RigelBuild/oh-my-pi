...
- An agent's `autoloadSkills` entry that matches no available skill is now reported instead of being dropped silently, so a misspelled name no longer leaves an agent looking configured while it autoloads nothing.
### Changed
