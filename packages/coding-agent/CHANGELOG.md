...
- A `restart` recycle now parks and invalidates only the recycling parent's own adopted subagents. When two top-level SDK sessions coexist, restarting either one parked the other's idle subagents and marked their revivers stale — so on the ordinary SDK path, where no persisted-subagent reviver factory is installed, every later revival refused a subagent whose own parent and captured dependencies were still perfectly live. A failed reattachment likewise no longer refuses another parent's parked subagents. A subagent whose owning session cannot be determined is still treated as belonging to every recycle, since an adoption that cannot be attributed must not be assumed safe.
