...
- A `restart` recycle now scopes its subagent parking to the restarting parent's own ownership chain. With two SDK top-level sessions sharing the process-global lifecycle, recycling one previously drained, parked, and stale-marked the *other* session's still-live children, so a later follow-up to them either could not revive or was rebuilt through the wrong replacement factory; the other session's subagents are now left untouched ([#10289](https://github.com/can1357/oh-my-pi/pull/10289) by [@mattwilkinsonn](https://github.com/mattwilkinsonn)).
