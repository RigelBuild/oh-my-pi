...
export interface AuthBrokerCommandArgs {
	...
	flags: {
		...
		fromLocal?: boolean;
		/** `serve`: path to the JSON subscription-config file (SEA-1242 Layer 1). */
...
/**
 * On-disk JSON shape for the subscription config (SEA-1242 Layer 1). `accounts`
 * is keyed by the opaque provider accountId (`accountLabelOf`); `plans` by
...
async function runServe(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	// Load the static subscription config (SEA-1242 Layer 1) if a path is set via
	// the flag or `OMP_AUTH_BROKER_SUBSCRIPTIONS`; a parse/shape error throws so
	// the broker never boots emitting partial series. Unset → omit, broker runs
	// exactly as before.
