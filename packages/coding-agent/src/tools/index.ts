...
export const BUILTIN_TOOLS: Record<BuiltinToolName, ToolFactory> = {
	...
	// itself the contract (OQ-9).
	refresh: s => new RefreshTool(s),
	// Conditional (createIf → null when session.requestRestart is unbound):
	// unlike refresh, restart is only offered when a host onRestartRequested
	// callback is wired, so an unbound session is not shown a tool that always
	// errors.
