...
export class AgentSession {
	 ...
	 * `onBeforeRefresh` pull surfaces to a draining restart (T6), not only to the
	...
	async refresh(scope: RefreshScope = "all"): Promise<RefreshResult> {
		...
		const run = this.#refreshTail.then((): RefreshResult | Promise<RefreshResult> => {
			if (this.#restartInFlight()) return { refused: "restarting" };
			// #activeRefresh is the raw #doRefresh run a draining restart (T6) reads
			// so a failed onBeforeRefresh pull surfaces to the restart, not just to
			// this caller — never the failure-swallowing tail below.
			...
			// onBeforeRefresh pull — which OMP's process-level handler escalates to
			...
			// `active`/`run` to the caller and (in T6) the drain.
			const clearActive = () => {
