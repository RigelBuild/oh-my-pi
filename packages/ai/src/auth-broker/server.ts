...
export interface AuthBrokerServerOptions {
	...
	/**
	 * Static subscription-config input (SEA-1242 Layer 1). Populated from sealed
	 * nix and threaded into the `/metrics` renderer as its `subscriptions` opts
	 * member, producing the four `llm_subscription_*` families. Absent → those
	 * families are not emitted and `/metrics` is byte-identical to before.
	 */
