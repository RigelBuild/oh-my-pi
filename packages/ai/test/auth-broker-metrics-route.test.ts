...
import { type AuthCredentialStore, AuthStorage, SqliteAuthCredentialStore } from "@oh-my-pi/pi-ai";
...
describe("auth-broker GET /metrics route", () => {
	...
	beforeEach(async () => {
		...
		// Upstream moved the aggregate-usage seam onto the credential store
		// (`AuthCredentialStore.fetchUsageReports`), which `UsageService.reports`
		// prefers over its per-credential fan-out.
		store.fetchUsageReports = signal => usageImpl(signal);
...
describe("auth-broker scrape-token validation is route-scoped", () => {
	...
	beforeEach(async () => {
		...
		store = await SqliteAuthCredentialStore.open(path.join(tempDir, "agent.db"));
		store.fetchUsageReports = async () => sampleReports();
		storage = new AuthStorage(store);
		await storage.credentials.reload();
	});
